...
3.	Run ‘training.py’ program for adding face images to collection ‘myfacecollection’. The program uses the ‘IndexFaces’ operation to detect faces in an image and add them to collection. Used ‘ExternalImageId’ field in the ‘IndexFace’ operation to associate a name with an image. The ‘ListFaces’ operation is then used to return faces stored in the collection
